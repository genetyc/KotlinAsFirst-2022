...
fun centerFile(inputName: String, outputName: String) {
    ...
    var max = 0
    c.forEach { max = maxOf(max, it.trim().length) }
    File(outputName).bufferedWriter().use {
        for (i in c.indices) {
            it.write(" ".repeat((max - c[i].trim().length) / 2))
            it.write(c[i].trim())
            it.newLine()
        }
    }
...
fun top20Words(inputName: String): Map<String, Int> {
    ...
    val c = File(inputName).readLines()
    if (c.isEmpty()) return mapOf()
    c.forEach {
        it.split(Regex("""[^а-яА-Яa-zA-ZёЁ]""")).forEach { word ->
            val lowered = word.lowercase()
            if (lowered in ans) ans[lowered] = ans[lowered]!! + 1
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    val c = File(inputName).readLines().associateWith {
        it.lowercase().toSet().size
    }.toMap()
    var mx = 0
    if (c.isNotEmpty()) {
        c.keys.forEach {
            if (it.length==it.lowercase().toSet().size) mx=maxOf(it.length, mx)
        }
    }
    val ms = mutableListOf<String>()
    for ((k, v) in c) if (v == mx && k.length == v) ms.add(k)
    File(outputName).bufferedWriter().use {
        it.write(ms.joinToString(separator = ", "))
    }
}
